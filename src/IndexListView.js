...
export default class CityIndexListView extends Component {
    ...
    _renderRightLetters(letter, index) {
        return (
            <TouchableOpacity key={'letter_idx_' + index} activeOpacity={0.6} onPress={() => { this._scrollTo(index, letter) }}>
    ...
    _renderListBox(cityJson, rowId) {
        return (
            <TouchableOpacity key={'list_item_' + cityJson.id} style={styles.rowViewBox} onPress={() => { this._cityNameClick(cityJson) }}>
    ...
    _renderListRow = (cityJson, rowId) => {
        ...
        if (rowId === key_now || rowId === key_hot || rowId === key_last_visit) {
            return this._renderListBox(cityJson, rowId);
        }

        return (
            <TouchableOpacity key={'list_item_' + cityJson.id} style={styles.rowView} onPress={() => { this._cityNameClick(cityJson) }}>
